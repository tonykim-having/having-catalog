...
function startLoadingSequence() {
  setLoadingText('Connecting to catalog data...');
...
function markLoadingStage(stage) {
  if (stage === 'fetched') {
    ...
    setLoadingProgress(Math.max(loadingProgress, 82));
  }
  if (stage === 'rendering') {
    setLoadingText('Preparing catalog...');
    setLoadingProgress(Math.max(loadingProgress, 92));
  }
  if (stage === 'done') {
...
function finishLoading() {
  ...
  setTimeout(() => {
    document.getElementById('loadingLayer')?.classList.add('hidden');
  }, 220);
}
...
function normalizeBrandsPayload(payload) {
  ...
  brandRows.forEach((row, index) => {
    ...
    const rawFilters = splitMulti(
      pick(row, ['filters', 'filter_tags', 'category_primary'])
    )
    ...
    const tag = asText(pick(row, ['short_description', 'tag']));
    const about = asText(pick(row, ['full_description', 'about', 'description']));
    ...
    BMOut[id] = {
      ...
      listImage: asText(
        pick(row, ['list_image_url', 'list_image', 'listImage', 'hero_image_url'])
      ),
      bgImage: asText(
        pick(row, ['hero_image_url', 'bg_image', 'bgImage', 'list_image_url'])
      )
    };
  ...
  const allFilterTags = uniq(
    BORDEROut.flatMap(id => (BMOut[id]?.filterTags || []).filter(Boolean))
  );
  ...
  return {
    BM: BMOut,
    BORDER: BORDEROut,
    FILTERS: FILTERSOut.length > 1 ? FILTERSOut : DEFAULT_FILTERS
  };
}
...
function normalizeCompanyPayload(payload) {
  ...
  return {
    ...
    address: asText(
      pick(company, ['address']),
      'B-519, 338, Gwanggyojungang-ro, Suji-gu, Yongin-si, Gyeonggi-do, 16942, Korea'
    ),
...
function exclTone(status) {
  ...
  switch (v) {
    case 'Open':
      return { label: 'Open', color: '#1f6f43' };
    case 'Selective':
      return { label: 'Selective', color: '#8a5a00' };
    case 'Restricted':
      return { label: 'Restricted', color: '#9b3d2f' };
    case 'Not Available':
      return { label: 'Not Available', color: '#6B6B67' };
    default:
      return { label: v || 'Case by Case', color: 'var(--dark)' };
  }
}
    ...
    <div class="info-card">
      ...
      <div class="info-supply">
        ${(m.supply || [])
          .map(
            s =>
              `<span class="info-badge" style="border-color:${m.accent};color:${m.accent}">${s}</span>`
          )
          .join('')}
      </div>
    </div>
    ...
    .map(
      ...
      <div class="sku-card" data-sku="${p.id}">
        <div class="sku-img">
          ${
            p.img
              ? `<img src="${p.img}" alt="${p.name}" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:contain;display:block;padding:18px;">`
              : ''
          }
        ...
        <div class="sku-body">
          ...
          <div class="sku-tags">
            ${(p.tags || []).map(t => `<span class="sku-tag">${t}</span>`).join('')}
          </div>
        </div>
...
function renderP2() {
  ...
  root.innerHTML = `
    <div class="p2-hero reveal in" id="brandHero" style="${heroStyle}">
      <div class="p2-top">
        <div class="p2-main-left reveal in">
          ...
          <div class="p2-tech-inline">
            ${(m.tech || [])
              .map(
                (t, i) => `
                <div class="p2-ti reveal in" data-delay="${i + 1}">
                  <div class="p2-ti-name">${t.n}</div>
                  <div class="p2-ti-desc">${t.d}</div>
                </div>
              `
              )
              .join('')}
        ...
        <div class="p2-meta-col reveal in">
          <div class="p2-mc">
            <div class="p2-mc-lbl">Channel</div>
            <div class="p2-mc-val">${m.channel}</div>
          </div>
          <div class="p2-mc">
            <div class="p2-mc-lbl">Total SKUs</div>
            <div class="p2-mc-val" style="font-family:var(--serif);font-size:20px;font-weight:300;color:${m.accent}">
              ${m.totalSku}
            </div>
          </div>
          <div class="p2-mc">
            <div class="p2-mc-lbl">Supply Mode</div>
            <div class="p2-supply">
              ${(m.supply || [])
                .map(
                  s =>
                    `<span class="p2-sbadge" style="border-color:${m.accent};color:${m.accent}">${s}</span>`
                )
                .join('')}
            </div>
          </div>
          <div class="p2-mc">
            <div class="p2-mc-lbl">Active Markets</div>
            <div class="p2-market-list">${renderMarketsChips(m.markets, 'p2-market-chip')}</div>
          </div>
          <div class="p2-mc">
            <div class="p2-mc-lbl">Exclusivity</div>
            <div class="p2-excl">
              <span class="p2-excl-badge" style="border-color:${exclTone(m.exclusivity.status).color};color:${exclTone(m.exclusivity.status).color}">
                ${exclTone(m.exclusivity.status).label}
              </span>
              <span class="p2-excl-note">${m.exclusivity.note || ''}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="sku-head">
        ...
        <div class="sku-filters">
          ${(m.filters || ['All'])
            .map(
              f =>
                `<button class="sfb${currentSkuFilter === f ? ' on' : ''}" data-sf="${f}">${f}</button>`
            )
            .join('')}
        </div>
      </div>
    ...
    <div class="brand-cta reveal">
      ...
      <button class="bcta-btn" style="background:${m.accent}" data-action="inquire-brand" data-brand="${currentBrand}">
        Inquire About This Brand &rarr;
      </button>
    </div>
  `;
...
async function goBrand(bid, opts = {}) {
  ...
  try {
    ...
    if (root) {
      root.innerHTML = `
        <div class="sku-wrap">
          <div style="padding:48px 0;text-align:center;font-size:13px;color:var(--mid)">
  ...
  document.getElementById('mBody').innerHTML = `
    <div class="mb-inner">
      <div class="mb-img">
        ${
          p.img
            ? `<img src="${p.img}" alt="${p.name}" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:contain;display:block;max-height:320px;">`
            : ''
        }
      ...
      <div class="mb-info">
        ...
        <div class="mb-ingr">
          <div class="mb-chips">
            ${(p.keys || []).map(k => `<span class="mb-chip">${k}</span>`).join('')}
          </div>
        </div>
        ...
        <div class="mb-feats">
          ${(p.feats || [])
            .map(
              f => `
              <div class="mb-feat">
                <div class="mb-feat-dot" style="background:${m.accent}"></div>
                <div class="mb-feat-text">${f}</div>
              </div>
            `
            )
            .join('')}
    ...
    <div class="mb-footer">
      ...
      <div class="mb-excl-row">
        <span class="mb-excl-badge" style="border-color:${exclTone(m.exclusivity.status).color};color:${exclTone(m.exclusivity.status).color}">
          ${exclTone(m.exclusivity.status).label}
        </span>
        <span class="mb-excl-note">${m.exclusivity.note || ''}</span>
      </div>
      <div class="mb-certs">
        ${(p.certs || [])
          .map(c => `<span class="mb-cert" style="border-color:${m.accent};color:${m.accent}">${c}</span>`)
          .join('')}
      </div>
      <button class="mb-cta" style="background:${m.accent}" data-action="inquire-product" data-product="${p.id}">
...
async function updateInquiryProducts(selectedBrands = [], selectedProducts = []) {
  ...
  sel.innerHTML = prods
    .map(p => `<option value="${p.id}">${BM[p.b].name} — ${p.name}</option>`)
    .join('');
